...
// Dashboard summary
router.get('/dashboard', requireCompany, (req, res) => {
    try {
    ...
    `).get(companyId);
        ...
        const monthlyQuotations = db.prepare(`
      SELECT COUNT(*) as count, COALESCE(SUM(grand_total), 0) as total
      FROM quotations WHERE company_id = ? AND date LIKE ?
    `).get(companyId, `${currentMonth}%`);

        const monthlyReceipts = db.prepare(`
      SELECT COUNT(*) as count, COALESCE(SUM(amount), 0) as total
    ...
    `).get(companyId);
        ...
        res.json({
            ...
            pendingBalance: pendingBalance.total,
            monthly: {
...
router.get('/summary', requireCompany, (req, res) => {
    try {
        ...
        // Quotation totals
        const quotationTotals = db.prepare(`
      SELECT 
        COUNT(*) as count,
...
// Bedroom-wise report
router.get('/bedroom-wise', requireCompany, (req, res) => {
    try {
        const { start_date, end_date } = req.query;
        const companyId = req.session.companyId;
...
// Project-wise report
router.get('/project-wise', requireCompany, (req, res) => {
    try {
        const { start_date, end_date } = req.query;
        const companyId = req.session.companyId;
...
router.get('/monthly-trend', requireCompany, (req, res) => {
    try {
        const companyId = req.session.companyId;
